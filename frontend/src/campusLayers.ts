...
const GRAPH_LAYER_IDS = new Set([
  'campus-pathways',
  'campus-pathways-stairs',
  'campus-stairs-fill',
  'campus-stairs-line',
  'campus-ramp',
])
...
export function applyOverlayVisibility(map: mapboxgl.Map, showGraph: boolean, showCampus: boolean) {
  addCampusLayers(map)
  for (const layer of CAMPUS_LAYERS) {
    const visible = GRAPH_LAYER_IDS.has(layer.id) ? showGraph : showCampus
