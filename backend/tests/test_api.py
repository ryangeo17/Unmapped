...
def test_accessibility_score_uses_jhu_grade(client):
    walk = route(client, "Malone Hall", "Clark Hall")
    assert walk["verified_stats"]["fully_compliant_percent"] is not None
    assert 40 <= walk["scores"]["accessibility"] <= 100
    assert any("pathway survey" in line for line in walk["explanation"])
...
def test_gps_coordinates_route_from_current_location(client):
    malone = next(item for item in client.get("/api/landmarks").json() if item["name"] == "Malone Hall")
    start = f"{malone['latitude']},{malone['longitude']}"
    result = route(client, start, "Clark Hall")
    assert result["geometry"]
    assert result["start_node"]


def test_unknown_current_location_name_is_still_rejected(client):
    response = client.post("/api/routes", json={"start": "Current location", "end": "Malone Hall"})
    assert response.status_code == 404


def test_admin_can_draw_one_verified_path_and_add_a_hazard(client):
    token = login(client)
    headers = {"Authorization": f"Bearer {token}"}
    empty = client.get("/api/admin/verified-path", headers=headers)
    assert empty.status_code == 200
    assert empty.json()["path"] is None

    malone = next(item for item in client.get("/api/landmarks").json() if item["name"] == "Malone Hall")
    clark = next(item for item in client.get("/api/landmarks").json() if item["name"] == "Clark Hall")
    midpoint = [
        (malone["latitude"] + clark["latitude"]) / 2,
        (malone["longitude"] + clark["longitude"]) / 2,
    ]
    saved = client.put(
        "/api/admin/verified-path",
        headers=headers,
        json={
            "geometry": [
                [malone["latitude"], malone["longitude"]],
                midpoint,
                [clark["latitude"], clark["longitude"]],
            ],
            "name": "Robot walk",
            "from_landmark": malone["id"],
            "to_landmark": clark["id"],
        },
    )
    assert saved.status_code == 200, saved.text
    path = saved.json()
    assert path["id"] == "e-robot-verified"
    assert path["verified"] is True
    assert path["from_landmark"] == malone["id"]
    assert path["to_landmark"] == clark["id"]
    assert path["from_name"] == "Malone Hall"
    assert path["to_name"] == "Clark Hall"
    assert len(path["geometry"]) >= 3

    searched = route(client, "Malone Hall", "Clark Hall")
    assert searched["edge_ids"] == ["e-robot-verified"]
    assert searched["verified_stats"]["verified_percent"] == 100
    assert route(client, "Clark Hall", "Malone Hall")["edge_ids"] == ["e-robot-verified"]
    ...
    assert again.json()["id"] == "e-robot-verified"

    far = client.post(
        "/api/admin/verified-path/hazards",
        headers=headers,
        json={
            "title": "Too far",
            "description": "Not on the path",
            "robot_note": "",
            "severity": 2,
            "kind": "roughness",
            "active_when": "always",
    ...
    hazard = client.post(
        ...
        headers=headers,
        json={
            "title": "Crack in the paver",
            "description": "Raised brick",
            "robot_note": "Depth about 2 cm.",
            "severity": 2,
            "kind": "roughness",
            "active_when": "night",
            "latitude": midpoint[0],
    ...
    body = hazard.json()
    assert body["active_when"] == "night"
    assert "2 cm" in body["robot_note"]
    ...
    assert "e-robot-verified" not in route(client, "Malone Hall", "Clark Hall")["edge_ids"]
